...
var (
	...
	mdMonoFont  = canvas.Font{Family: "Consolas", Size: 12.5}
)

// mdExpandTabs 把 tab 展开为 4 空格（等宽字体下 tab 会渲染成缺字框，且 Go 代码多 tab 缩进）。
func mdExpandTabs(s string) string { return strings.ReplaceAll(s, "\t", "    ") }
...
// mdRender 把 markdown 文本渲染为深色块级 widget（标题按字号分级 / 列表 / 引用 / 分隔线 / 代码块 / 段落）。
func mdRender(text string) widget.Widget {
	...
	for _, raw := range strings.Split(text, "\n") {
		...
		switch {
		case t == "---" || t == "***":
			blocks = append(blocks, widget.Div(widget.Style{Height: 1, BackgroundColor: ghBorder}))
		case strings.HasPrefix(t, "### "):
			blocks = append(blocks, label(mdStripInline(t[4:]), ghText, 13.5))
		case strings.HasPrefix(t, "## "):
			blocks = append(blocks, label(mdStripInline(t[3:]), ghText, 15))
		case strings.HasPrefix(t, "# "):
			blocks = append(blocks, label(mdStripInline(t[2:]), ghText, 17))
		case strings.HasPrefix(t, "> "):
		...
		default:
			if m := mdOrderedRe.FindStringSubmatch(t); m != nil {
				...
				blocks = append(blocks, label(mdStripInline(t), ghText, 13))
			}
		}
	}
...
func mdQuote(text string) widget.Widget {
	return widget.Div(widget.Style{FlexDirection: "row", AlignItems: "stretch", Gap: 8},
		widget.Div(widget.Style{Width: 3, BackgroundColor: ghBorder, BorderRadius: 1.5}),
		expand(label(text, ghTextSec, 13)),
	)
}

func mdListItem(bullet, text string) widget.Widget {
	return widget.Div(widget.Style{FlexDirection: "row", Gap: 8},
		label(bullet, ghTextSec, 13),
		expand(label(text, ghText, 13)),
	)
}
...
func mdCodeBlock(lines []string, lang string) widget.Widget {
	...
	rows := widget.HighlightCode(mdExpandTabs(joined), lang) // 显示用：tab→空格（等宽字体下 tab 渲染成缺字框）
	...
	if lang != "" || widget.ClipboardWrite != nil { // 头：语言标签 + 复制（剪贴板可用才放，避免空按钮）
		hdr := []widget.Widget{}
		if lang != "" {
			hdr = append(hdr, label(lang, ghTextMuted, 10))
		}
		hdr = append(hdr, expand(widget.Div(widget.Style{})))
		if widget.ClipboardWrite != nil {
			hdr = append(hdr, &widget.Button{
				Icon: "copy", IconSize: 12, TextColor: ghTextMuted,
	...
	return widget.Div(
		widget.Style{BackgroundColor: ghBgPrimary, BorderColor: ghBorder, BorderWidth: 1, BorderRadius: 4,
